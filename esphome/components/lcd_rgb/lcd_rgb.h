...
class RGBLCDDisplay : public lcd_base::LCDDisplay, public i2c::I2CDevice {
 public:
  void set_writer(std::function<void(RGBLCDDisplay &)> &&writer) { this->writer_ = std::move(writer); }
  void setup() override;
 ...
 protected:
  ...
  std::function<void(RGBLCDDisplay &)> writer_;
};
